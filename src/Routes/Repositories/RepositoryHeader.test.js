...
describe('Repository Header', () => {
  it('should render correctly', async () => {
    ...
    expect(subTitle.firstChild.textContent).toEqual(
    ...
    await waitFor(() => {
      expect(
        screen.getByTestId('repository-header-popover-title').innerHTML
      ).toEqual('About Fleet Management');
      expect(
        screen.getByTestId('repository-header-popover-body').innerHTML
      ).toEqual(
        'Fleet Management is a service that allows you to provision, update and maintain edge systems.'
      );
      expect(
        screen.getByTestId('repository-header-popover-footer').firstChild
          .innerHTML
